...
#[derive(Debug)]
pub enum PikuseruError {
    PluginError
}

impl Display for PikuseruError {
    fn fmt(&self, fmt: &mut Formatter) -> FmtResult {
        match self {
            PikuseruError::PluginError => write!(
                fmt,
...
pub fn draw_logo(screen: &mut gfx::Screen) {
    ...
    screen
        .print(format!("Powered by Pikuseru {:?}.{:?}.{:?}", VERSION, MAJOR_VERSION, MINOR_VERSION).to_string(),
        0,
        (height-16) as i32,
        7);
}
...
impl PikuseruCartridge {
    pub fn new(cartridge: Cartridge, filename: String) -> PikuseruCartridge {
        PikuseruCartridge {
    ...
    pub fn empty(filename: String, full_filename: String) -> PikuseruCartridge {
        PikuseruCartridge {
            filename: filename.clone(),
...
impl fmt::Debug for PikuseruCartridge {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(f,
               "Pikuseru Cartridge {{ cart: {:?} }}",
               self.cartridge)
    }
}
...
pub struct Pikuseru {
    ...
    pub debug: bool,
    pub cartridge: PikuseruCartridge,
    pub state: PikuseruState,
    pub fps: f64,
    pub record: Record,
    pub version: u32,
...
impl Pikuseru {
    pub fn new() -> Pikuseru {
        info!("[Pikuseru] Creating new Pikuseru");
        ...
        Pikuseru {
            ...
            cartridge: PikuseruCartridge::zero(),
           
            state: PikuseruState::STOP,
    ...
    pub fn toggle_debug(&mut self) {
        info!("[Pikuseru] Toggle Debug {:?}", self.debug);
    ...
    pub fn reset(&mut self) {
        info!("[Pikuseru] Reset");
        self.cartridge = PikuseruCartridge::zero();
        self.state = PikuseruState::STOP;
        self.debug = false;
    ...
    pub fn start_record(&mut self, filename: &str) {
        warn!("[Pikuseru][Record] Record feature not enable (image feature)");
        self.record.recording = false;
    }

    #[cfg(feature = "image")]
    pub fn start_record(&mut self, filename: &str) {
        info!("[Pikuseru][Record] Start to record the frame");
    ...
    pub fn record(&mut self) {
        info!("[Pikuseru][Record] Recording the frame {:?}", self.record.images.len());
    ...
    pub fn stop_record(&mut self) -> Result<()> {
        ...
        info!("[Pikuseru][Record] Stop to record the frame {:?}",
        ...
        for i in 0..self.record.images.len() / (screen.width * screen.height * 4) {
            info!("[Pikuseru][Record] Generate frame {:?} {:?}/{:?}",
                  i,
                  idx,
                  self.record.images.len());
            ...
            info!("[Pikuseru][Record] Creating ImageBuffer {:?}", buffer.len());

            let image =
                image::ImageBuffer::from_raw(screen.height as u32, screen.width as u32, buffer)
                    .unwrap();

            info!("[Pikuseru][Record] Rotating image");
            let image = image::imageops::rotate270(&image);
            let image = image::imageops::flip_vertical(&image);
            

            info!("[Pikuseru][Record] Creating gif Frame");
            let mut frame = image::Frame::new(image);
            encoder.encode_frame(frame).unwrap();
        }

        info!("[Pikuseru][Record] GIF created in {:?}", self.record.filename);
        Ok(())
    }

    #[cfg(not(feature = "image"))]
    pub fn screenshot(&mut self, filename: &str) {
        warn!("[Pikuseru] Screenshot feature not enable (image feature)")
    }
    ...
    pub fn screenshot(&mut self, filename: &str) {
        ...
        info!("[Pikuseru] Taking screenshot {:?}x{:?} in {:?}", screen.width, screen.height, filename);
    ...
    pub fn save_current_cartridge(&mut self) {
        ...
        info!("[Pikuseru][SAVE] Saving the current cartridge in {:?}",
              output_filename);

        info!("[Pikuseru][SAVE] Set the new sprites");
        self.cartridge.cartridge.gfx.set_sprites(screen.sprites.clone());
        info!("[Pikuseru][SAVE] Set the new map");
        self.cartridge.cartridge.map.set_map(screen.map.clone());
        info!("[Pikuseru][SAVE] Set the new flags");
        self.cartridge.cartridge.gff.set_flags(screen.sprites.clone());
        //info!("[Pikuseru][SAVE] Set the new palette");
        //screen.palette.set_colors(screen.palettes.get_colors());

        match self.cartridge.cartridge.format {
            CartridgeFormat::PikuseruFormat => {
                self.cartridge.cartridge.save_in_pikuseru(&output_filename,
                                                format!("{:?}.{:?}.{:?}",
                                                        self.version,
                                                        self.major_version,
                                                        self.minor_version)
    ...
    pub fn _load_cartridge(&mut self)
        ...
        match self.cartridge.get_code_type() {
            Code::LUA => {
                info!("[Pikuseru] Loading LUA Plugin");
            ...
            Code::RPYTHON => {
                info!("[Pikuseru] Loading RPYTHON Plugin");
            ...
            Code::PYTHON => {
                info!("[Pikuseru] Loading PYTHON Plugin");
            ...
            Code::RHAI => {
                info!("[Pikuseru] Loading RHAI Plugin");
            ...
            Code::WASM => {
                info!("[Pikuseru] Loading WASM Plugin");
        ...
        info!("[Pikuseru] CARTRIDGE LOADED SUCCESSFULLY");

        Ok(())
    }

    pub fn load_cartridge(&mut self, filename: String) -> bool {
        info!("[Pikuseru] Load cartridge from {:?}", filename);
        ...
        if filename.contains(".corn") {
            ...
            match Cartridge::from_png_file(filename.as_str()) {
                Ok(c) => cartridge = c,
                Err(e) => panic!("[Pikuseru] Impossible to load the Pico8 PNG cartridge {:?}", e),
            }
        } else if filename.contains(".p8") {
            match Cartridge::from_p8_file(filename.as_str()) {
                Ok(c) => cartridge = c,
                Err(e) => panic!("[Pikuseru] Impossible to load the Pico8 P8 cartridge {:?}", e),
            }
        } else {
            panic!("[Pikuseru] Unknown file format !");
        }

        self.cartridge = PikuseruCartridge::new(cartridge, filename);
        self.resize_buffer(self.cartridge.cartridge.mode_width, self.cartridge.cartridge.mode_height);
        self._setup_screen();

        match self._load_cartridge() {
            Ok(()) => self.state = PikuseruState::RUN,
            Err(err) => error!("[Pikuseru] [Impossible to load the cartridge]: {}", err),
        }
    ...
    pub fn call_init(&mut self) {
        info!("[Pikuseru] CALL INIT {:?} {:?}", self.cartridge.get_code_type(), self.state);

        if self.state == PikuseruState::RUN {
            match self.cartridge.get_code_type() {
                ...
                _ => error!("[Pikuseru] Impossible to match a plugin"),
            }
        }
    }
            ...
            match self.cartridge.get_code_type()  {
                Code::LUA => {
                    match self.cartridge.lua_plugin.draw() {
                        Ok(()) => (),
                        Err(err) => error!("[Pikuseru] [call_draw / lua]: {}", err),
                    }
                }
                Code::RPYTHON => {
                    match self.cartridge.rpython_plugin.draw() {
                        Ok(()) => (),
                        Err(err) => error!("[Pikuseru] [call_draw / rpython]: {}", err),
                    }
                }
                Code::RHAI => {
                    match self.cartridge.rhai_plugin.draw() {
                        Ok(()) => (),
                        Err(err) => error!("[Pikuseru] [call_draw / rhai]: {}", err),
                    }
                }
                Code::PYTHON => {
                    match self.cartridge.python_plugin.draw() {
                        Ok(()) => (),
                        Err(err) => error!("[Pikuseru] [call_draw / python]: {}", err),
                    }
                }
                Code::WASM => {
                    match self.cartridge.wasm_plugin.draw() {
                        Ok(()) => (),
                        Err(err) => error!("[Pikuseru] [call_draw / wasm]: {}", err),
                    }
                }
            ...
            match self.cartridge.get_code_type() {
                Code::LUA => {
                    match self.cartridge.lua_plugin.update() {
                        Ok(()) => (),
                        Err(err) => error!("[Pikuseru] [call_update / lua]: {}", err),
                    }
                }
                Code::RPYTHON => {
                    match self.cartridge.rpython_plugin.update() {
                        Ok(()) => (),
                        Err(err) => error!("[Pikuseru] [call_update / rpython]: {}", err),
                    }
                }
                Code::RHAI => {
                    match self.cartridge.rhai_plugin.update() {
                        Ok(()) => (),
                        Err(err) => error!("[Pikuseru] [call_update / rhai]: {}", err),
                    }
                }
                Code::PYTHON => {
                    match self.cartridge.python_plugin.update() {
                            Ok(()) => (),
                            Err(err) => error!("[Pikuseru] [call_update / python]: {}", err),
                        }
                }
                Code::WASM => {
                    match self.cartridge.wasm_plugin.update() {
                        Ok(()) => (),
                        Err(err) => error!("[Pikuseru] [call_update / wasm]: {}", err),
                    }
                }
                _ => (),
            }
